	...
	<div class="kt-subheader-search ">
		<div class="kt-container  kt-container--fluid ">
			<h3 class="kt-subheader-search__title">
				Payment Status
				<span class="kt-subheader-search__desc">Payment Status list</span>
			</h3>
		</div>
	</div>
				...
				<div class="kt-portlet__head-toolbar">
					<div class="kt-portlet__head-wrapper">
						...
						<div class="kt-portlet__head-actions">
							<a href="{{ route('admin.lead.payment-status.create') }}" class="btn btn-brand btn-elevate btn-icon-sm">
				...
				<table class="table table-striped- table-bordered table-hover table-checkable" id="event_type_tbl">
					...
					<tbody>
						...
						<tr>
							...
							<td>{{ $payment->venue_name }}</td>
							<td>{{ $payment->amount }}</td>
							<td>{{ $payment->received_payment }}</td>
							<td>{{ $payment->remaining_amount }}</td>
							<td>{{ $payment->payment_date }}</td>
							<td>
								<a href="{{ route('admin.lead.payemnt-status.edit',$payment->id) }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Edit">
									<i class="la la-edit"></i>
								</a>

								<button class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Delete" value="{{ $payment->id }}" id="delete">
	...
	$(document).ready(function(){
		...
		$(document).on('click', '#delete', function ()
		{
			...
			function () {
				$.ajax({
					type: "POST",
					url: "{{ route('admin.lead.payment-status.delete') }}",
					data: 
					{
