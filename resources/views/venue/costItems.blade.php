		...
		<div class="kt-portlet kt-portlet--mobile">
				...
				<div class="kt-portlet__head-toolbar">
					<div class="kt-portlet__head-wrapper">
						<div class="kt-portlet__head-actions">
							<a href="{{ route('admin.add.CostItems',$new_id) }}" class="btn btn-brand btn-elevate btn-icon-sm">
			...
			<div class="kt-portlet__body">
				<table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_1">
					...
					<tbody>
						@foreach($costitems as $items)
						<tr id="{{ $items->id }}">
							<td>{{ ($items->item_name) }}</td>
							<td>{{ ($items->price) }}</td>
							<td>{{ ($items->des) }}</td>
							<td>
								<a href="{{ url('admin/edit/CostItems'.'/'.$items->id.'/'.$new_id) }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Edit">
									<i class="la la-edit"></i>
								</a>
								<button type="button" class="btn btn-sm btn-clean btn-icon btn-icon-md" data-id="{{ route('admin.CostItemsdelete',$items->id) }}" title="Delete" id="delete_costitems">
...
<script>
	...
	$(document).ready(function(){
		...
		$(document).on('click', '#delete_costitems', function ()
		{
			var obj = $(this);
